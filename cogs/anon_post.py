    ...
    async def anon(self, interaction: discord.Interaction, message: str):
        ...
        if category_name in CATEGORIES:
            await interaction.response.send_message(
                "Posted anonymously", delete_after=15.0, ephemeral=True
            )
            ...
            await mod_channel.send(
                f"{interaction.user.name} sent an anonymous message in {interaction.channel.name}: {message}"
            )
        else:
            await interaction.response.send_message(
                f"You can only post anonymously in the following categories: {', '.join(CATEGORIES)}",
                delete_after=15.0,
                ephemeral=True,
            )
