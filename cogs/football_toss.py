...
class FootballTossCog(commands.Cog):
	...
	def _name_for(self, count: int) -> str:
		return 'FootballHolder' if count == 1 else f'FootballHolder{min(self.limit, count)}'


	async def _get_role(self, interaction: discord.Interaction, count: int) -> discord.Role:
		return discord.utils.get(interaction.guild.roles, name=self._name_for(count))


	async def _get_count(self, interaction: discord.Interaction, user: discord.Member) -> int:
		user_roles = user.roles
		football_roles = [discord.utils.get(interaction.guild.roles, name=self._name_for(count)) for count in range(1, self.limit + 1)]
	...
	async def throw(self, interaction: discord.Interaction, receiver: discord.Member, item: str=':football:'):
		await self.do_throw(interaction, receiver, item, 'throw')


	@app_commands.command(
		name='toss',
		description='Toss something to another user'
	)
	async def toss(self, interaction: discord.Interaction, receiver: discord.Member, item: str=':football:'):
		await self.do_throw(interaction, receiver, item, 'toss')


	async def do_throw(self, interaction: discord.Interaction, receiver: discord.Member, item: str, action: str):
		async def add_one(n):
		...
		if interaction.channel.name != 'footballtoss':
			await interaction.response.send_message('You must be in the #footballtoss channel', delete_after=30.0)
			return
		...
		if thrower.top_role.id not in self.PRIVILEGED_THROWERS and count == 0:
			await interaction.response.send_message(f"{thrower.mention} can't {action} something they don't have")
			...
			if receiver.bot:
				await interaction.response.send_message(f'{thrower.mention} just tried to {action} something at a bot')
				await add_one('total_fails')
			else:
				...
				await interaction.response.send_message(f'{thrower.mention} {verb} a {item} at {receiver.mention}')
				await add_one('total_throws')
		await self._update_status()


async def setup(bot):
	await bot.add_cog(FootballTossCog(bot), guilds=[discord.Object(id=Constants.GUILD_ID)])
